...
/**
 * Class Luxury stores information about truck - daily price, booked days, maximum occupancy, if it
...
public class Luxury extends Vehicle {
  ...
  private static final int LUXURY_MAXIMUM_OCCUPANCY = 4;
  private static final boolean LUXURY_REQUIRE_SPECIALIZED_LICENSE = false;
  private static final boolean LUXURY_ALLOW_DIFFERENT_PICKUP_AND_DROPOFF_LOCATIONS = false;
  private Integer maximumOccupancy;
  private Boolean requireSpecializedLicense;
  private Boolean allowDifferentPickupAndDropoffLocations;

  /**
   * Constructor for the class Luxury
   *
   ...
   */
  public Luxury(Double dailyPrice, Integer bookedDays)
      throws DailyPriceNumericException, BookedDaysNumericException {
    super(dailyPrice, bookedDays);
    this.maximumOccupancy = LUXURY_MAXIMUM_OCCUPANCY;
    this.requireSpecializedLicense = LUXURY_REQUIRE_SPECIALIZED_LICENSE;
    this.allowDifferentPickupAndDropoffLocations = LUXURY_ALLOW_DIFFERENT_PICKUP_AND_DROPOFF_LOCATIONS;
  }
  ...
  /**
   * Set the luxury's number of booked days to the given value if the booking is valid
   *
   * @param days                  - the number of days a client would want to reserve a luxury, as
   *                              an Integer
